...
function getAlunoData() {
    ...
    .then(data => {
        ...
        const bancoId = data.banco.id;
        
        // Salvar o bancoId em uma variável global para usar em outro lugar
        console.log("ID do banco: ", bancoId);

        // Chamar outra função para usar o bancoId se necessário
        processarBancoId(bancoId);  // Exemplo de como usar o bancoId
    })
    .catch(error => {
...
async function getBancos() {
    try {
        const response = await fetch(`http://localhost:8080/bancos/${userId}`, {
            method: "GET",
            headers: {
